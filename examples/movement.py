...
logger.info("Connecting to CRI server at 127.0.0.1:3921...")

# Connect to the simulator at 127.0.0.1 with port 3921
if not controller.connect("127.0.0.1", 3921):
    logger.error("Unable to connect to iRC! Check if the simulator is running.")
    quit()
...
if not controller.robot_state.active_control:
    logger.warning("Active control is NOT enabled! Check connection.")
    controller.close()
    quit()
else:
    logger.info("Active control enabled! You can now send commands.")

# Enable the motors
if not controller.enable():
    logger.error("Error: Unable to enable motors. Check the simulator.")
    controller.disable()
    controller.close()
    quit()
...
logger.info(f"Releasing brake using DOUT {BRAKE_DOUT}...")
...
logger.info(f"Current Kinematics State: {controller.robot_state.kinematics_state}")
...
if controller.robot_state.kinematics_state not in KinematicsState.__members__.values():
    logger.warning("Unexpected Kinematics State received. Motion might not be allowed.")
    controller.disable()
    controller.close()
    quit()
...
if not controller.wait_for_kinematics_ready(10):
    logger.error("Error: Kinematics not ready. Check the simulator.")
    controller.disable()
    controller.close()
    quit()

logger.info("Kinematics Ready. Moving to start position...")
...
if not controller.move_joints(A1=0, A2=0, A3=0, A4=0, A5=0, A6=0,
    ...
    logger.error("Error: Movement to zero failed! Check simulator settings.")
    controller.disable()
    controller.close()
    quit()

logger.info("Successfully moved to zero position.")
...
if not controller.move_joints_relative(A1=10, A2=10, A3=5, A4=0, A5=0, A6=0,
    ...
    logger.error("Error: Relative movement failed! Check simulator settings.")
    controller.disable()
    controller.close()
    quit()

logger.info("Successfully completed relative movement.")

# Pause, stop, and disable the robot
sleep(2)
controller.pause_programm()
logger.info("Paused the program.")

sleep(2)
controller.stop_programm()
logger.info("Stopped the program.")
...
if controller.robot_state.operation_mode != OperationMode.NOT_ENABLED:
    ...
    controller.disable()
    logger.info("Motors disabled successfully.")
else:
    logger.info("Robot is already disabled, skipping disable command.")
